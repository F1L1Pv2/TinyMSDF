...
int main() {
    ...
    pixels = (uint32_t*)malloc(INNER_SIZE * INNER_SIZE * 4);
    ...
    for(int i = 0; i < INNER_SIZE*INNER_SIZE; i++){
        int x = i % INNER_SIZE;
        int y = INNER_SIZE - i / INNER_SIZE;
        vec2 P = {((float)x+0.5f)/INNER_SIZE, ((float)y+0.5f)/INNER_SIZE};
        pixels[i] = generatePixel(test,P);
    }


    if(IMAGE_WIDTH == INNER_SIZE && IMAGE_HEIGHT == INNER_SIZE){
        outPixels = pixels;
    }else{
        outPixels = (uint32_t*)malloc(IMAGE_WIDTH * IMAGE_HEIGHT * 4);
        stbir_resize_uint8((const unsigned char*)pixels,INNER_SIZE,INNER_SIZE,INNER_SIZE*4,(unsigned char*)outPixels,IMAGE_WIDTH,IMAGE_HEIGHT,IMAGE_WIDTH*4,4);
    }

    collisionCorrection(outPixels, IMAGE_WIDTH, IMAGE_HEIGHT);

    if(!stbi_write_png("out.png", IMAGE_WIDTH, IMAGE_HEIGHT, 4, outPixels, IMAGE_WIDTH * 4)){
